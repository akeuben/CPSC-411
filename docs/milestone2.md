# Milestone 2 - Avery Keuben (30170731)
...
## Code Qualities
1. *Code is readable:* 
    - Shaping rules are split into seperate categories, with spare lines seperating subcategories into readable chunks. 
    - Parser related files have been split into multiple different files (parser.py, errors.py, shaping.py, shape.py) to seperate different components and make single files easier to take in and understand.
2. *Code is well documented:* Comments exist for non-obvious implementation details, including:
    - various shaping rules
    - overriden functions in the ASTShaper to perform some light post-processing, including why it is neccessary to perform these extra steps, as well as how they are performed.
    - Doc comments are added to each class method (Lexer and Parser) to ensure understanding of the usage of each method. 
3. *Code is modular and well-structured.* As with the lexer (see milestone 1 document `./milestone.md` in git repo) the parser is also wrapped in a class. This allows for a custom interface into the parser and seperates the antlr4 code from our python code. This allows for the antlr4 parser to be replaced in the future without signiticant changes to the API of the parser. Further, as described above, the parser and lexer were split into multiple different files to seperate different secations to make reading the code trivial.
4. *Code does not contain unnecessary duplication.* Common functionality between the lexer and parser are located in the `src/core` directory, and is not duplicated for each of the lexer and parser.
...
6. *Speed* See run output. Code runs in a reasonable amount of time. The parser doesn't introduce any loops or recursion (outside of libraries) and makes use of the provided libraries which are assumed to be efficient. 
