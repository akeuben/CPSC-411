...
[x] A non-void function must return a value.
[x] A value returned from a function has the wrong type.
[x] No return statements at all are present in a non-void function. Note that you're only checking for the existence of an appropriate return statement at the semantic checking stage, not whether it's actually executed.
...
[x] An integer literal is out of range.
[x] The main declaration can't have parameters (if not handled during parsing).
[x] A statementexpression can only be an assignment or function invocation (if not handled during parsing).
"""
